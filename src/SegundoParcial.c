	...
	while(opcion != 7  ){
		...
		switch(opcion){
			...
			case 1:// CARGAR ARCHIVOS
				//
				 if(controller_loadFromText("listaServicios.csv", listaServicios) == 1){
			...
			case 4:// FILTRAR POR TIPO
				...
				if(size > 0){
					...
					switch (opcionServicio){
						...
						case 1://MINORISTA
							//llamo a la funcion ll_filter() para que reciba una lista y una función que guardara el tipo seleccionado. .
					...
					if(sizeFilter > 0 ){
						...
						//llamo a la funcion que me crea el archivo de texto con los servicios filtrados.
			...
			case 5://MOSTRAR SERVICIOS
				//llamo a la funcion ll_len() para que retorne el tamaño del LinkedList.
				size = ll_len(listaServicios);

				//si la lista esta vacia. muestra un mensaje de error.
				if(size > 0){
					//llamo a la funcion ll_sort() para que se genere el ordenamiento .
					ll_sort(listaServicios, eServicio_sortByDescripcion, 1);
